...
_sg={};
_sg.Functions={};
_sg.Users={			--Players now have two different profile type.
	Profiles={};
	Accounts={};
	Groups={};
};
_sg.Commands={};	--Store all commands on the server, including "client-side" only.
_sg.Modules={};		--Contains "Modules" used to extend system features.
_sg.Registry={		--Replacement of the "Data" system. Will be more robust and powerful overall.
	["SYSTEM"]={
		ServerVersion=1.0;
		ServerBuild=100001;
	};
...
To interact with either the server or client, an active connection needs to
be opened. If a connection is not active for a certain period of time,
the events will be disconnected and the RemoteEvent will be destroyed.
This is a new security mechanism to help prevent exploiting of
remote connections.

Session Types:
Sessions is the type of connection that will be opened on the
requested session. The supported sessions are the following:

intsess : Starts an interactive session between server and client.
Server can execute code onto the client and receive responses.

datatns : Starts an session that allows transferring data between
server and client. NOTE: Code cannot be executed between sessions.
]]--

_sg.Functions.OpenConnection=function(id,name,session,sevnt)
	--TODO: Add "session" types for type of communication.
	--TODO: Allow for customized "Event" function for processing packets.
	...
	local request_str=_sg.Functions.Encrypt("%sgreq/intsess/"..name,'test');
	--Encrypt our request string. Confuses users wanting to snoop.
	
	--We have to ask the client to make a RemoteEvent for communication.
	Create'StringValue'{
		Name="Sess_"..#cq:GetChildren()+1;
		Value=request_str;
		Archivable=false;
		Parent=cq;
	};
	...
	return {
		Event=cnt;
		Connections={
			Server=cnt.OnServerEvent:connect(function(player,packet,hash)
				pcall(function() --Prevents errors from breaking connection.
					...
					debug("Checking packet for proper decryption...")
					if type(pk)=='string' then
						debug("Packet decrypted properly. Converting to table.");
						local tbl=JSONDecode(pk);
						local hsh=_sg.Functions.Hash(tbl.Packet);
						debug("Checking hash for verification...");
						if hsh==hash then
							--The hash is verified. Executing code.
							debug("Verified packet. Compiling for execution.");
							local func,err=loadstring(tbl.Packet);
							if not err then
								debug("No errors. Executing...");
								...
								if not suc then
									debug("Packet Execution Error: "..tostring(res));
								else
									debug("Successfully executed packet.");
								end;
							else
								debug("Compiling error has occurred.");
								debug(tostring(err));return;
							end;
						else
							debug("The packet failed to verify against hash. Ignoring packet.");return;
						end;
					else
		...
		PushPacket=function(self,packet)
			...
			debug("Pushed packet to client.");
			return true;
		end;
		CloseConnection=function(self)
			if self==nil then error("Must call :Disconnect(), not .Disconnect()");end;
			debug("Closing connection \""..self.Event.Name.."\"...")
			for i,v in next,self.Connections do
				debug("Attempting to close connection "..tostring(i));
				local suc,res=pcall(function() v:disconnect() end);
				if suc then					
					debug("Connection closed successfully.");
				else
					debug("Connection failed to close. Result: "..tostring(res));
				end;
			end;
			debug("Destroying RemoteEvent...");
			PDestroy(self.Event);
			debug("Closed connection \""..self.Event.Name.."\".");
			return true;
		end;
	};
end
...
_sg.Functions.CreateGroup=function(Name,Control,Desc)
	if type(_sg.Users.Groups[Name])=='table' then
		_sg.Users.Groups[Name]={
			GroupName=Name;
...
_sg.Functions.RemoveGroup=function(Name)
	if type(_sg.Users.Groups[Name])=='table' then
		_sg.Users.Groups[Name]=nil;
		return true;
	else
...
_sg.Functions.GetLowestGroup=function()
	local Max = math.huge
	for i,v in next,_sg.Users.Groups do
		if v.ControlContext<Max then
			Max=v.ControlContext;
		end;
	end;
	return v;
end;

_sg.Functions.GetHighestGroup=function()
	local Low=-math.huge;
	for i,v in next,_sg.Users.Groups do
		if v.ControlContext>Low then
			Low=v.ControlContext;
		end;
	end;
	return v;
end;
